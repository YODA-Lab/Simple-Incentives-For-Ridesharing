...
from Vehicle import Vehicle, VehicleLocation
...
class Oracle(object):
    """An Oracle returns a list of feasible actions for a given Vehicle."""
    ...
    def get_feasible_actions(self,
        ...
        """Get a list of the best feasible actions for each Vehicle."""

        # Associate requests with closest MAX_TRIPS_SIZE_1 vehicles
        def _distance_to_request(veh: Vehicle, request: Request):
            return self.envt.get_travel_time(veh.position.next_location, request.pickup) + veh.position.time_to_next_location

        MAX_TRIPS_SIZE_1 = min(MAX_TRIPS_SIZE_1, len(vehs))
        requests_for_each_veh: List[List[Request]] = [[] for _ in range(len(vehs))]
        for request_idx, request in enumerate(requests):
            times_to_pickup = [(veh_idx, _distance_to_request(veh, request)) for veh_idx, veh in enumerate(vehs)]
            times_to_pickup.sort(key=lambda x: x[1])
            for idx in range(MAX_TRIPS_SIZE_1):
                veh_idx = times_to_pickup[idx][0]
                requests_for_each_veh[veh_idx].append(request)

        # Get feasible trips for each vehicle
        feasible_actions_all_vehs = []
        for requests_for_veh, veh in zip(requests_for_each_veh, vehs):
            trips: List[Action] = []
            tested_actions: Set[Action] = set()
            num_is_feasible_calls = 0

            # Create null action
            null_action = Action([], veh.id)
            null_action.new_path = veh.path
            trips.append(null_action)

            # Check feasibility for individual requests
            for request in requests_for_veh:
                action = Action([request], veh.id)
                action.new_path = self.get_new_path(veh, veh.path, request)
            ...
            while len(nodes_to_expand) > 0 and num_is_feasible_calls < MAX_IS_FEASIBLE_CALLS:
                ...
                for trip_size_1_idx in trips_size_1:
                    ...
                    new_action = Action(prev_requests.union(new_requests), veh.id)
                    ...
                    if (new_action not in tested_actions):
                        ...
                        new_path = self.get_new_path(veh, action.new_path, new_request)
            ...
            feasible_actions_all_vehs.append(trips)

        return feasible_actions_all_vehs

    def get_new_path(self, veh: Vehicle, current_path: Path, new_request: Request, SEARCH_THRESHOLD: int = -1) -> Optional[Path]:
    ...
    def get_new_path_complete_search(self, veh: Vehicle, path: Path) -> Path:
        ...
        while True:
            ...
            if (current_index < len(possible_next_nodes)):
                ...
                if not current_request_order:
                    ...
                    current_time = self.envt.current_time + veh.position.time_to_next_location
    ...
    def get_new_path_insertion(self, veh: Vehicle, path: Path) -> Path:
        # DROPOFF
        relevant_request_id = len(path.requests) - 1  # Assume the new request is the last one
        new_node = PathNode(True, relevant_request_id)
        path, insert_idx = self._insert_path(veh, path, new_node)

        # PICKUP
        if (insert_idx != Path.NOT_APPLICABLE):
            new_node = PathNode(False, relevant_request_id)
            path, _ = self._insert_path(veh, path, new_node, insert_idx)

        return path

    def _insert_path(self, veh: Vehicle, path: Path, new_node: PathNode, max_insert_idx: int = -1) -> Tuple[Path, int]:
        ...
        for insert_idx in range(len(path.request_order), -1, -1):
            delay, delay_for_new_node, min_future_delay = self._can_insert_node(veh, path, new_node, insert_idx, min_future_delay)
            ...
            if (insert_idx <= max_insert_idx):
                ...
                if (not is_dropoff):
                    _, _, _, current_capacity = self._get_node_info(veh, path, insert_idx - 1)
        ...
        if (min_delay_idx != Path.NOT_APPLICABLE):
            ...
            self._insert_pathnode(veh, path, new_node, min_delay_idx, min_individual_delay, max_insert_idx)
            # Update total_delay
    ...
    def _get_node_info(self, veh: Vehicle, path: Path, idx: int):
        if (idx != Path.NOT_APPLICABLE):
            ...
            location = veh.position.next_location
            deadline = float('inf')
            visit_time = self.envt.current_time + veh.position.time_to_next_location
    ...
    def _can_insert_node(self,
        ...
        prev_location, prev_deadline, visit_time, current_capacity = self._get_node_info(veh, path, insert_idx - 1)
    ...
    def _insert_pathnode(self,
        ...
        if (min_delay_idx == 0):
            ...
            prev_time = self.envt.current_time + veh.position.time_to_next_location
            prev_loc = veh.position.next_location
        else:
    ...
    def get_path_insertion_2(self, veh: Vehicle, path: Path) -> Path:
        ...
        for dropoff_idx in range(len(path.request_order), -1, -1):
            dropoff_delay, new_node_delay, dropoff_future_delay = self._can_insert_node(veh, path, dropoff['new_node'], dropoff_idx, dropoff['min_future_delay'])
            ...
            if (dropoff_delay != Path.NOT_APPLICABLE):
                ...
                for pickup_idx in range(dropoff_idx, -1, -1):
                    pickup_delay, _, pickup['min_future_delay'] = self._can_insert_node(veh, path, pickup['new_node'], pickup_idx, pickup['min_future_delay'])
                    ...
                    # Special check for capacity constratints
                    _, _, _, current_capacity = self._get_node_info(veh, path, pickup_idx - 1)
                    if (current_capacity >= self.envt.MAX_CAPACITY):
                        break
        ...
        if (max_total_delay != Path.NOT_APPLICABLE):
            # Dropoff
            self._insert_pathnode(veh, path, dropoff['new_node'], dropoff['min_delay_idx'], dropoff['min_individual_delay'])
            # Pickup
            self._insert_pathnode(veh, path, pickup['new_node'], pickup['min_delay_idx'], pickup['min_individual_delay'], dropoff['min_delay_idx'])
            # Update total_delay
            path.total_delay = max_total_delay
